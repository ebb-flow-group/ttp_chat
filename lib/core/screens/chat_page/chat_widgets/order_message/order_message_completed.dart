...
class OrderMessageCompleted extends StatelessWidget {
  final CustomMessage message;

  const OrderMessageCompleted({Key? key, required this.message}) : super(key: key);
    ...
    if (isCreatorApp) {
      return OrderMessageBox(
        title: RichText(
          ...
          text: TextSpan(
            text: 'Nice! Order ${message.metadata?['id']} has been ',
            style: Ts.t3Bold(Config.primaryColor),
            children: [TextSpan(text: 'completed', style: Ts.t2Bold(Config.grayG1Color))],
          ),
        ),
        subtitle: RichText(
          textAlign: TextAlign.center,
          text: TextSpan(text: 'Enjoy your order!', style: Ts.t3Reg(Config.grayG1Color)),
        ),
        actions: [
          AppButton(
            text: 'View Order',
